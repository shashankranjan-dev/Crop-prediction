...
def compare(conditions=["N", "P", "K", "temperature", "ph", "humidity", "rainfall"]):
    ...
    print(
        "Black Grams : {0: .2f}".format(
            data[(data["label"] == "blackgram")][conditions].mean()
        )
    )
    print(
        "Banana : {0: .2f}".format(data[(data["label"] == "banana")][conditions].mean())
    )
    ...
    print(
        "Papaya : {0: .2f}".format(data[(data["label"] == "papaya")][conditions].mean())
    )
    print(
        "Muskmelon : {0: .2f}".format(
            data[(data["label"] == "muskmelon")][conditions].mean()
        )
    )
    print(
        "Grapes : {0: .2f}".format(data[(data["label"] == "grapes")][conditions].mean())
    )
    print(
        "Watermelon : {0: .2f}".format(
            data[(data["label"] == "watermelon")][conditions].mean()
        )
    )
    print(
        "Kedney Beans : {0: .2f}".format(
            data[(data["label"] == "kidneybeans")][conditions].mean()
        )
    )
    print(
        "Mung Beans : {0: .2f}".format(
    ...
    print(
        "Cotton : {0: .2f}".format(data[(data["label"] == "cotton")][conditions].mean())
    )
    print(
        "Maize : {0: .2f}".format(data[(data["label"] == "maize")][conditions].mean())
    )
    print(
        "Moth Beans : {0: .2f}".format(
            data[(data["label"] == "mothbeans")][conditions].mean()
        )
    )
    print(
        "Pigeon peas : {0: .2f}".format(
            data[(data["label"] == "pigeonpeas")][conditions].mean()
        )
    )
    print(
        "Mango : {0: .2f}".format(data[(data["label"] == "mango")][conditions].mean())
    )
    print(
        "Pomegrante : {0: .2f}".format(
            data[(data["label"] == "pomegrante")][conditions].mean()
        )
    )
    print(
        "Coffee : {0: .2f}".format(data[(data["label"] == "coffee")][conditions].mean())
    )
...
print(
    "Crops which requires very High rainfall:",
    data[data["rainfall"] > 200]["label"].unique(),
)
print(
    "Crops which requires very Low rainfall:",
    data[data["rainfall"] < 40]["label"].unique(),
)
